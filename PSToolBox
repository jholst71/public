...
Function Get-LatestReboot { ### Get-LatestReboot - Get Latest Reboot / Restart / Shutdown for logged on server
  ...
  $host.UI.RawUI.WindowTitle = "Get latest Shutdown / Restart / Reboot for Local Server - Events After: $($FEventLogStartTime)";
...
Function Get-LatestRebootDomain {
  ...
  ## Script
  $FEventLogStartTime = [DateTime]::Now.AddDays(-$($FLastXDays)).AddHours(-$($FLastXHours));
  $host.UI.RawUI.WindowTitle = "Get latest Shutdown / Restart / Reboot for multiple Domain Servers - Events After: $($FEventLogStartTime)";
